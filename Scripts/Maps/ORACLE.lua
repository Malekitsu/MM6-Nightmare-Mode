...
evt.house[3] = 170  -- "The Oracle"
evt.map[3] = function()
	evt.EnterHouse(170)         -- "The Oracle"
end
...
evt.map[5] = function()
	...
	if Party[Game.CurrentPlayer].Class%3==1 then
		...
		Party[Game.CurrentPlayer].Class=Party[Game.CurrentPlayer].Class+1
		Message(string.format("As the Crystal rests upon the pedestal, an enigmatic force surges through %s' body, enveloping him in a maelstrom of power. The intensity of this surge is both exhilarating and agonizing, pushing him to the brink of his endurance. The sheer magnitude of this energy is so overwhelming that %s knows he could never withstand such an experience again.",Party[Game.CurrentPlayer].Name,Party[Game.CurrentPlayer].Name))
	else if Party[Game.CurrentPlayer].Class%3==0 then
			Message("You need a 3rd promotion before being able to handle the Crystal")
			return

		else 
			Message(string.format("As you attempt to place the Crystal on the pedestal once more, a foreboding aura permeates the air. Unbeknownst to you, the previous surge of power had taken its toll on your body, leaving you vulnerable and unable to withstand another infusion. Your body, unable to endure the overwhelming force, succumbs to its might, and you are eradicated, leaving behind a cautionary tale for those who dare to tread the same path.",Party[Game.CurrentPlayer].Name))
			Party[Game.CurrentPlayer].HP=0
			Party[Game.CurrentPlayer].Eradicated=1
			return
		end
	end		
::_36::
...
::_5::
	...
	--ADD 2nd PROMOTION FOR 255 MODE
	if Party[Game.CurrentPlayer].Class%3==1 then	
		evt.Subtract("Inventory", 551)         -- "Memory Crystal Beta"
		evt.Set("QBits", 101)         -- Oracle
		evt.Subtract("QBits", 192)         -- Quest item bits for seer
